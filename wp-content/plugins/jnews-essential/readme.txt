Change Log :

== 11.0.5 ==
- [IMPROVEMENT] Add Widget Importer & Exporter plugin integration
- [BUG] Fix show upload user profile only if user have permission to upload file

== 11.0.4 ==
- [IMPROVEMENT] Add tiktok into user social info

== 11.0.3 ==
- [BUG] Fix: fix custumizer style issue

== 11.0.2 ==
- [IMPROVEMENT] Update framework

== 11.0.1 ==
- [IMPROVEMENT] Update framework

== 11.0.0 ==
- [IMPROVEMENT] Compatible with JNews v11.0.0

== 10.0.9 ==
- [BUG] Fix upload button on metabox not showing the preview.

== 10.0.8 ==
- [IMPROVEMENT] Update framework

== 10.0.7 ==
- [IMPROVEMENT] add backend custom profile picture

== 10.0.6 ==
- [BUG] Fix font variance and subset issue

== 10.0.5 ==
- [IMPROVEMENT] Add Google Analytics 4 support

== 10.0.4 ==
- [IMPROVEMENT] Add Twitch social author

== 10.0.3 ==
- [BUG] update script

== 10.0.2 ==
- [BUG] Fix toggle button Vafpress issue

== 10.0.1 ==
- [BUG] Fix RTL mode issue
- [BUG] Fix: JNews post metabox revert to default value

== 10.0.0 ==
- [IMPROVEMENT] Compatible with JNews v10.0.0

== 9.0.2 ==
- [BUG] Fix: Undefined index warning message

== 9.0.1 ==
- [IMPROVEMENT] Update framework

== 9.0.0 ==
- [IMPROVEMENT] Compatible with JNews v9.0.0

== 8.0.4 ==
- [IMPROVEMENT] Update framework, capability to reverse compare string array with "in" or "contains" on active callback

== 8.0.3 ==
- [IMPROVEMENT] Update framework

== 8.0.2 ==
- [BUG] Fix sanitize data

== 8.0.1 ==
- [IMPROVEMENT] Update framework

== 8.0.0 ==
- [IMPROVEMENT] Compatible with JNews v8.0.0

== 7.0.5 ==
- [IMPROVEMENT] Update framework

== 7.0.4 ==
- [BUG] Fix jQuery.browser deprecated issue

== 7.0.3 ==
- [BUG] Fix classic editor issue on WordPress 5.5

== 7.0.2 ==
- [BUG] Fix classic editor issue on WordPress 5.5

== 7.0.1 ==
- [BUG] Fix error notice on PHP 7.4

== 7.0.0 ==
- [IMPROVEMENT] Compatible with JNews v7.0.0

== 6.0.2 ==
- [BUG] Fix issue with implode() function PHP 7.4

== 6.0.1 ==
- [IMPROVEMENT] Update analytics script code

== 6.0.0 ==
- [IMPROVEMENT] Compatible with JNews v6.0.0

== 5.0.3 ==
- [IMPROVEMENT] Use .on()

== 5.0.2 ==
- [IMPROVEMENT] Testing with Envato Theme Check plugin

== 5.0.1 ==
- [IMPROVEMENT] Move framework to JNews Essential

== 5.0.0 ==
- [IMPROVEMENT] Compatible with JNews v5.0.0

== 4.0.1 ==
- [BUG] Fix header builder issue with Polylang

== 4.0.0 ==
- [IMPROVEMENT] Compatible with JNews v4.0.0

== 3.0.0 ==
- [IMPROVEMENT] Compatible with JNews v3.0.0

== 2.0.0 ==
- [IMPROVEMENT] Compatible with JNews v2.0.0

== 1.0.0 ==
- First Release
