Change Log :

== 11.0.1 ==
- [BUG] Fix JNews Gallery on Autoloaded Post Issue

== 11.0.0 ==
- [IMPROVEMENT] Compatible with JNews v11.0.0

== 10.0.2 ==
- [BUG] Fix Undefined key warning

== 10.0.1 ==
- [BUG] Fix Gallery section on RTL mode

== 10.0.0 ==
- [IMPROVEMENT] Compatible with JNews v10.0.0

== 9.0.5 ==
- [BUG] Fix gallery missing arrow icon

== 9.0.4 ==
- [BUG] Fix gallery caption issue
- [BUG] Fix issue next/prev after switch fullscreen

== 9.0.3 ==
- [BUG] Fix gallery not rendering

== 9.0.2 ==
- [IMPROVEMENT] Improve script loader without boost mode

== 9.0.1 ==
- [BUG] Fix gallery preview issue

== 9.0.0 ==
- [IMPROVEMENT] Compatible with JNews v9.0.0

== 8.0.1 ==
- [IMPROVEMENT] Improve best practices and accessibility
- [BUG] Fix square preloader issue

== 8.0.0 ==
- [IMPROVEMENT] Compatible with JNews v8.0.0

== 7.0.3 ==
- [BUG] Fix style drag image issue

== 7.0.2 ==
- [improvement] replace .unbind() with .off()
- [improvement] replace .bind() with .on()

== 7.0.1 ==
- [BUG] Fix media library issue on WordPress 5.5

== 7.0.0 ==
- [IMPROVEMENT] Compatible with JNews v7.0.0
- [BUG] Fix save issue on block editor
- [BUG] Fix script loaded on AMP page

== 6.0.1 ==
- [IMPROVEMENT] Only load the style and script file when needed
- [IMPROVEMENT] Add lazyload image for image ad

== 6.0.0 ==
- [IMPROVEMENT] Compatible with JNews v6.0.0

== 5.0.2 ==
- [BUG] Fix navigation arrow

== 5.0.1 ==
- [IMPROVEMENT] Use .on()

== 5.0.0 ==
- [IMPROVEMENT] Compatible with JNews v5.0.0
- [NEW FEATURE] Double click expand for JNews Gallery

== 4.0.0 ==
- [IMPROVEMENT] Compatible with JNews v4.0.0

== 3.0.2 ==
- [BUG] Fix fullscreen mode issue

== 3.0.1 ==
- [IMPROVEMENT] Remove all white space on Google ad ID

== 3.0.0 ==
- [IMPROVEMENT] Compatible with JNews v3.0.0

== 2.0.0 ==
- [IMPROVEMENT] Compatible with JNews v2.0.0

== 1.0.0 ==
- First Release
